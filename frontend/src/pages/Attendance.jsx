...
export default function Attendance() {
  ...
  useEffect(() => {
    ...
    if (clockedIn && clockInTime) {
      interval = setInterval(() => setElapsed(Math.floor((Date.now() - clockInTime) / 1000)), 1000);
    }
  ...
  const handleClockOut = () => {
    ...
    const newRecord = {
      ...
      clockIn: inTime.toTimeString().slice(0,5),
      clockOut: now.toTimeString().slice(0,5),
      hours,
      status: inTime.getHours() <= 9 ? 'ontime' : 'late',
    };
    const updated = [newRecord, ...records];
    setRecords(updated);
    localStorage.setItem('wp_attendance', JSON.stringify(updated));
    setClockedIn(false);
    setClockInTime(null);
    setElapsed(0);
  };
    ...
    <div style={{ display:'flex', minHeight:'100vh' }}>
      ...
      <main style={{ flex:1, padding:'32px', overflowY:'auto' }}>
        <div style={{ marginBottom:'28px' }}>
          ...
          <p style={{ color:'#7b7a99', fontSize:'14px', marginTop:'4px' }}>Track your daily attendance</p>
        </div>

        {/* Clock In/Out */}
        <div className="card" style={{ marginBottom:'24px', textAlign:'center', padding:'40px' }}>
          <p style={{ fontSize:'48px', fontFamily:'monospace', fontWeight:800, color: clockedIn ? '#00d9a3' : '#f0eeff', marginBottom:'8px', letterSpacing:'4px' }}>
            {clockedIn ? fmt(elapsed) : new Date().toLocaleTimeString('en-IN', { hour:'2-digit', minute:'2-digit', second:'2-digit' })}
          </p>
          <p style={{ color:'#7b7a99', fontSize:'14px', marginBottom:'28px' }}>
            {clockedIn ? `Clocked in at ${new Date(clockInTime).toLocaleTimeString('en-IN', { hour:'2-digit', minute:'2-digit' })}` : new Date().toLocaleDateString('en-IN', { weekday:'long', day:'numeric', month:'long', year:'numeric' })}
          </p>
          {!clockedIn
            ? <button className="btn-primary" onClick={handleClockIn} style={{ padding:'14px 48px', fontSize:'16px' }}>🟢 Clock In</button>
            : <button onClick={handleClockOut} style={{ background:'rgba(255,92,135,0.15)', border:'1px solid rgba(255,92,135,0.4)', color:'#ff5c87', padding:'14px 48px', fontSize:'16px', fontWeight:700, borderRadius:'8px', cursor:'pointer' }}>🔴 Clock Out</button>
          }
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px' }}>
          ...
          <div className="card">
            ...
            <div style={{ display:'flex', gap:'16px', marginTop:'12px' }}>
              <span style={{ fontSize:'11px', color:'#7b7a99', display:'flex', alignItems:'center', gap:'6px' }}><span style={{ width:'10px', height:'10px', borderRadius:'2px', background:'rgba(0,217,163,0.3)', display:'inline-block' }}/>On Time</span>
              <span style={{ fontSize:'11px', color:'#7b7a99', display:'flex', alignItems:'center', gap:'6px' }}><span style={{ width:'10px', height:'10px', borderRadius:'2px', background:'rgba(255,179,71,0.3)', display:'inline-block' }}/>Late</span>
              <span style={{ fontSize:'11px', color:'#7b7a99', display:'flex', alignItems:'center', gap:'6px' }}><span style={{ width:'10px', height:'10px', borderRadius:'2px', background:'#6c63ff', display:'inline-block' }}/>Today</span>
            </div>
          </div>
