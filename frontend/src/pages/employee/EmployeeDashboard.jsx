...
export default function EmployeeDashboard() {
  ...
  // Doughnut: app usage
  const appMap = {};
  activities.forEach(a => { appMap[a.appName] = (appMap[a.appName] || 0) + a.durationSeconds; });
  const topApps = Object.entries(appMap).sort((a,b)=>b[1]-a[1]).slice(0,5);
  const doughnutData = {
      ...
      <main style={{ flex:1, padding:'32px', overflowY:'auto' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'28px' }}>
          ...
          <button
            ...
            style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 20px', borderRadius:'10px', border:'1px solid', borderColor: trackingPaused ? 'var(--accent2)' : 'var(--accent3)', background: trackingPaused ? 'rgba(255,92,135,0.1)' : 'rgba(0,217,163,0.1)', color: trackingPaused ? 'var(--accent2)' : 'var(--accent3)', cursor:'pointer', fontWeight:600, fontSize:'14px' }}>
            <span>{trackingPaused ? '⏸ Tracking Paused' : '▶ Tracking Active'}</span>
          </button>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'16px', marginBottom:'24px' }}>
          ...
          <KPICard label="Apps Used"      value={topApps.length}      change="Today"                     color="var(--accent4)" changeType="neutral" />
